(function (global, fn) {
  ...
  frameWork.settings.initializeAccordion =
    frameWork.settings.initializeAccordion || true;
  ...
  __f.dayFormatNamesShort = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

  __f.dayFormatNamesShorter = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
  ...
  __f.dateToParse = (date) => {
    ...
    if (date) {
      if (Object.prototype.toString.call(date) === '[object Date]') {
        ...
        const pattern = new RegExp(__f.datetimeFormatPresets.Value.pattern);
  ...
  frameWork.validateBr = (breakpoint, mode) => {
    ...
    switch (mode) {
      case 'below': //max-width
        return document.documentElement.clientWidth <= __f.br_vals[breakpoint];

      case 'within':
        return (
          document.documentElement.clientWidth <= __f.br_vals[breakpoint] &&
          document.documentElement.clientWidth > __f.br_vals[__f.br_arr[currIndex - 1]]
        );
      ...
      case 'above':
        return currIndex > 0
          ...
          : document.documentElement.clientWidth > __f.br_vals[__f.br_arr[currIndex]];
    }
  };
  ...
  __f.dateIsValid = (date, args, rangeOnly) => {
    ...
    if (
      (__f.dateToParse(args.max) && __f.dateToParse(args.max) < d) ||
      (__f.dateToParse(args.min) && d < __f.dateToParse(args.min))
    ) {
  ...
  __f.createCalendarUi = (inputCalendar, valueForUi, args) => {
    if (inputCalendar) {
      ...
      const generateArrow = (buttonClass) => {
        ...
        switch (buttonClass) {
          case 'prev-month':
            ...
            arrowDate = __f.dateToVal(__f.dateGetAdjacent(currentCalendarDate, -1));
            validness = __f.dateIsValid(new Date(currYear, currMonth, 0), args, true);
            break;

          case 'prev-year':
            symbolClass = 'symbol-arrow-double-left';
            arrowDate = __f.dateToVal(__f.dateGetAdjacent(currentCalendarDate, -12));
            ...
            listItem = `<li class="${currClass}">
								<a href="#"
									...
									data-value="${__f.dateToVal(listItemDate)}">
										${__f.monthFormatNamesShort[listItemDate.getMonth()]} ${listItemDate.getFullYear()}
								</a>
							${
      ...
      const currPrevLastDate = (() => {
        ...
        freeGridSpacePrev =
          (currentCalendarDate.getDay() - parseInt(args.startDay) + 7) % 7,
        ...
        for (let i = 0; i < freeGridSpacePrev; i++) {
          ...
          let dateBlockPrev = generateBlock(
            ...
            `${__f.uiPrefix('calendar')}block-adjacent
							${!__f.dateIsValid(loopDatePrev, args) ? 'disabled' : ''}`
          );
  ...
  __f.tagsToParse = (value, returnWithInput) => {
    ...
    toReturn.forEach((tag, i) => {
      if (!tag || tag == '' || (tag === __f.tagsInputString && !returnWithInput)) {
        toReturn.splice(i, 1);
      }
    });
  ...
  frameWork.updateTags = (inputTags, allowFilter, newValue, valueForUi, inputText) => {
    let theValue = newValue || (inputTags.value !== '' && inputTags.value) || '';
      ...
      if (args.callbackNameFilter && allowFilter) {
        ...
        if (typeof fnToFilter === 'function') {
          const applyFilter = (valueToFilter, filterFnName) => {
            const inputIndex = __f
                .tagsToParse(valueToFilter)
            ...
            if (inputIndex > -1) {
              toReturn.splice(
                inputIndex < __f.tagsToParse(valueToFilter).length - 1
                  ? inputIndex
                  : toReturn.length,
                0,
                __f.tagsInputString

...
class Tabs extends FwComponent {
  ...
  UIChildren() {
    return super
      .UIEl()
      .querySelectorAll(`.${COMPONENT_CHILDREN_CLASS}, ${COMPONENT_CHILDREN_TAG}`);
  }
  ...
  UIActive() {
    return super
      .UIEl()
      .querySelector(
        `.${COMPONENT_CHILDREN_CLASS}.${ACTIVATED_CLASS}, ${COMPONENT_CHILDREN_TAG}.${ACTIVATED_CLASS}`
      );
  }
  ...
  static destroyListeners() {
    FwEvent.removeListener(document.documentElement, EVENT_CLICK, Tabs.handleClick());
  }
}

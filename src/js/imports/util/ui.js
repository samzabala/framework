...
import {
  lookupResetToParentClass,
  lookupResetFromClosestComponent,
  lookupResetFromClosestComponentUi,
} from './validation.js';
...
export const UIToggled = (toggleMode, triggerer, selector) => {
  ...
  const componentClass = `${FwString.ToDashed(
    toggleMode.replace('-open', '').replace('-close', '')
  )}`;
  ...
  const selectorToMatch = selector ? selector : `.${componentClass}`;
  ...
  if (triggerer) {
    ...
    if (
      ...
      document.querySelector(triggerer.getAttribute('href')).matches(selectorToMatch)
    ) {
...
export const UIToggleGroup = (
  ...
  //dem siblongs
  const selectorSiblings = FwDom.getSiblings(element).filter((sibling) => {

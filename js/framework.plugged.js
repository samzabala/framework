...
(function($,window){
	...
	frameWork.updateCalendar = function(inputCalendar,newValue,valueForUi){
		...
		var theValue = newValue || _.dateToVal(inputCalendar.attr('value'));
	...
	_.tagsToParse = function(value,returnWithInput){
		...
		returnWithInput = returnWithInput || true;

		toReturn = value.split(',');
	...
	_.tagsToVal = function(value,returnWithInput){
		...
		var toReturn = _.tagsToParse(value, returnWithInput).toString();
		console.warn(
			'to vals returns',
	...
	_.createTagsUi =  function(inputTags,valueForUi,args){
		if(inputTags){
			...
			if(!theUi.input.length) {
				theUi.wrapper.append('<input type="text" class="input '+uiPrefix()+'input">');
				theUi.input = theUi.wrapper.children('.'+uiPrefix()+'input');
			}
			...
			valArr.forEach(function(tag,i){
				//get index of input
				if(tag !== _.tagsInputString){
					insertMeth = (i < inputIn ) ? 'before' : 'after';
		
					
					theUi.input[insertMeth](function(){
						return '<span class="'+uiPrefix()+'tag"><span class="'+uiPrefix()+'tag-text">'+tag+'</span><a data-value-index="'+i+'" class="'+uiPrefix()+'tag-close" href="#"><i class="symbol symbol-close"></i></a></span>';
					})
				}
			})
	...
	frameWork.updateTags = function(inputTags,newValue,valueForUi){
		
		var theValue = newValue || inputTags.attr('value');

		valueForUi = valueForUi || _.tagsToVal(theValue,true) || '';
		
		// ignoreInput = ignoreInput || false;

		console.warn('funky fresh values',newValue,theValue, valueForUi);



		var arr =  {
			maxWidth: inputTags.attr('data-tags-max-width'),
			overflow: inputTags.attr('data-tags-overflow'),
		};
		...
		if(inputTags){
			_.createTagsUi(inputTags,valueForUi,args);
			
			//update the actual butt
			inputTags.attr('value',theValue);
		}
	...
	$(document).ready(function(){
		...
		$('body').on('click','.input-tags-ui .input-tags-ui-tag-close',function(e){	
			...
			inputTags = clicked.closest('.input-tags-ui').children('.input-tags');

			var tagToRemove = clicked.attr('data-value-index');

			newValue  = _.tagsToParse( inputTags.attr('value') ).splice(parseInt(tagToRemove,1));

			console.log('deldel', inputTags.attr('value'), tagToRemove,inputTags);
		...
		$('body').on('focus','input[data-toggle="dropdown"]',function(e){
